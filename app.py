...
# =========================================================
st.set_page_config(
    page_title="Spendwise · Finance Manager",
    page_icon="💳",
    layout="wide",
...
# =========================================================
# PRE-COMPILED REGEX (for PDF extraction)
# =========================================================
DATE_PATTERNS = [
...
# =========================================================
# AUTO-INSIGHTS ENGINE (for PDF data)
# =========================================================
...
with col_logo:
    ...
    st.markdown('<div class="tagline">Finance Manager · Transaction Analyzer + Budget Tracker</div>', unsafe_allow_html=True)
...
app_mode = st.radio(
    "Select Mode",
    ["📊 Transaction Analyzer", "💰 Budget Tracker"],
    horizontal=True,
...
if app_mode == "📊 Transaction Analyzer":
    ...
    with tab2:
        ...
        with pc1:
            ...
            if not debit_agg.empty:
                ...
                st.plotly_chart(fig, use_container_width=True, key="outflow_donut")
            else:
        ...
        with pc2:
            ...
            if not credit_agg.empty:
                ...
                fig.update_traces(textposition="inside", textinfo="percent+label",
                                  marker_line_width=0, hovertemplate="<b>%{label}</b><br>₹%{value:,.0f}<extra></extra>")
                fig.update_layout(**PLOTLY_DARK)
                st.plotly_chart(fig, use_container_width=True, key="inflow_donut")
            else:
        ...
        if not top_receivers.empty:
            ...
            st.plotly_chart(fig_bar, use_container_width=True, key="top_receivers")
    ...
    with tab4:
        ...
        # Check if we need to reinitialize tagging dataframes
        need_init = False
...
# =========================================================
# MODE 2: BUDGET TRACKER (Manual Input)
# =========================================================
else:  # "💰 Budget Tracker"
    ...
    with st.expander("➕ Add New Transaction", expanded=True):
        ...
        with col2:
            category = st.text_input("Category", placeholder="e.g., Groceries, Salary")
        with col3:
            amount = st.number_input("Amount (₹)", min_value=0.0, step=100.0)
        with col4:
            description = st.text_input("Description (optional)", placeholder="Brief note")
        with col5:
            st.write("")
            st.write("")
            if st.button("➕ Add", type="primary", use_container_width=True):
                if category and amount > 0:
                    st.session_state.budget_transactions.append({
                    ...
                    st.success(f"Added {transaction_type}: {category} - ₹{amount:,.2f}")
                    st.rerun()
                else:
                    st.error("Please enter a category and amount greater than 0.")
    
    # ── Display Transactions ──
    if st.session_state.budget_transactions:
        transactions_df = pd.DataFrame(st.session_state.budget_transactions)
        ...
        <div class="kpi-grid">
          <div class="kpi-card green">
            ...
            <div class="kpi-sub">{len(transactions_df[transactions_df["type"] == "Income"])} transactions</div>
          </div>
          <div class="kpi-card red">
            <div class="kpi-label">Total Expenses</div>
            <div class="kpi-value">{fmt_inr(total_expenses)}</div>
            <div class="kpi-sub">{len(transactions_df[transactions_df["type"] == "Expense"])} transactions</div>
          </div>
          <div class="kpi-card {'green' if net_savings >= 0 else 'red'}">
            <div class="kpi-label">Net Savings</div>
            <div class="kpi-value">{'+ ' if net_savings >= 0 else '− '}{fmt_inr(abs(net_savings))}</div>
            <div class="kpi-sub">{'Surplus' if net_savings >= 0 else 'Deficit'}</div>
          </div>
          <div class="kpi-card purple">
            <div class="kpi-label">Total Transactions</div>
            <div class="kpi-value">{len(transactions_df)}</div>
            <div class="kpi-sub">{transactions_df["category"].nunique()} unique categories</div>
          </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with viz_col1:
            ...
            if not expense_df.empty:
                expense_agg = expense_df.groupby("category")["amount"].sum().reset_index()
                fig = px.pie(expense_agg, values="amount", names="category", hole=0.55,
                             color_discrete_sequence=["#f43f5e","#fb7185","#fda4af","#fecdd3","#7c3aed","#a78bfa","#c4b5fd"])
                fig.update_traces(textposition="inside", textinfo="percent+label",
                                  marker_line_width=0, hovertemplate="<b>%{label}</b><br>₹%{value:,.0f}<extra></extra>")
                fig.update_layout(**PLOTLY_DARK)
        ...
        with viz_col2:
            ...
            if not income_df.empty:
                income_agg = income_df.groupby("category")["amount"].sum().reset_index()
                fig = px.pie(income_agg, values="amount", names="category", hole=0.55,
                             color_discrete_sequence=["#10b981","#34d399","#6ee7b7","#a7f3d0","#7c3aed","#a78bfa","#c4b5fd"])
                fig.update_traces(textposition="inside", textinfo="percent+label",
                                  marker_line_width=0, hovertemplate="<b>%{label}</b><br>₹%{value:,.0f}<extra></extra>")
                fig.update_layout(**PLOTLY_DARK)
        ...
        st.markdown('<div class="section-header">Income vs Expenses by Category</div>', unsafe_allow_html=True)
        if not transactions_df.empty:
            ...
            fig_bar = go.Figure(data=[
                go.Bar(name="Income", x=pivot_df["category"], y=pivot_df["Income"], 
                       marker_color="#10b981", opacity=0.8),
                go.Bar(name="Expense", x=pivot_df["category"], y=pivot_df["Expense"], 
                       marker_color="#f43f5e", opacity=0.8)
            ])
            fig_bar.update_layout(barmode="group", **PLOTLY_DARK)
        ...
        st.info("No transactions added yet. Start by adding your income and expenses above!")

# =========================================================
# FOOTER
# =========================================================
st.markdown("---")
st.caption("💳 Spendwise · Finance Manager · Data stays local and private")
